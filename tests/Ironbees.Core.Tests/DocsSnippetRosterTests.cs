...
/// assemblies next to this test (the Ironbees packages, the sibling packages they integrate and the
...
public class DocsSnippetRosterTests
{
    ...
    private static readonly HashSet<string> KnownExternal = new(StringComparer.Ordinal)
    {
        ...
        "GetSecretAsync", "Console", "File", "Information",
        // OpenAI SDK, in the FAQ
        "GetChatClient",
    };
    ...
    {
        "OpenAIClientOptions", // OpenAI SDK, in the FAQ
    };
    ...
    public void EveryNameADocumentUses_Exists_ExceptTheKnownDrift()
    {
        ...
        Assert.True(blocks > 40, "the scan must find the documentation's C# blocks, or an empty finding list proves nothing");
    ...
    public void APhantomProperty_AndAPhantomType_AreReported()
    {
        ...
        Assert.Equal<string>(["ImaginaryOptions (no such type in the library assemblies)", "IronbeesOptions.Nested", "IronbeesOptions.NoSuchOption"], findings.Order(StringComparer.Ordinal));
    }
    ...
    public void AMethodTheDocumentDefinesItself_IsNotDrift_ButAnUndefinedOneStillIs()
    {
        ...
        Assert.Contains("AddMyEmbedding", defined);
        Assert.Equal(["AddSomebodyElses"], calls);
    }
    ...
    public void APhantomRegistrationCall_IsReported_AndARealOneIsNot()
    {
        ...
        var calls = RegistrationCalls(
            "services.AddIronbees().ToString().UseImaginaryThing();\n" +
        ...
        Assert.Equal<string>(["AddIronbees", "NoSuchProcessAsync", "ToString", "ToUpperInvariant", "UseImaginaryThing"], calls.Order(StringComparer.Ordinal));
        Assert.Equal<string>(["NoSuchProcessAsync", "UseImaginaryThing"], calls.Where(n => !methods.Contains(n)).Order(StringComparer.Ordinal));
    }
    ...
    /// <summary>
    /// The assemblies whose public method names count as "exists": the Ironbees packages, the sibling
    ...
    [
        "Ironbees*.dll", "IronHive*.dll", "Microsoft.Agents*.dll", "Microsoft.Extensions.*.dll",
    ];
    ...
    private static Dictionary<string, List<Type>> OptionTypes() =>
        LoadedAssemblies("Ironbees*.dll")
            .Where(a => !a.GetName().Name!.EndsWith(".Tests", StringComparison.Ordinal) && !a.GetName().Name!.Contains(".Samples", StringComparison.Ordinal))
